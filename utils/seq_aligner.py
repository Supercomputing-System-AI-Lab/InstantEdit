...
def get_mapper(x: str, y: str, specifier, tokenizer, encoder, device, max_len=77):
    ...
    return alpha_e.to(device)


def get_local_mapper(prompts, specifiers, tokenizer, encoder, device, max_len=77):
    x_seq = prompts[0]
    alpha_objs = []
    for i in range(1, len(prompts)):
        alpha_obj = get_mapper(x_seq, prompts[i], specifiers[i-1], tokenizer, encoder, device, max_len)
        alpha_objs.append(alpha_obj)
    return torch.stack(alpha_objs)
