...
class PeRFlowScheduler(SchedulerMixin, ConfigMixin):
    ...
    def get_window_alpha(self, timepoints):
        ...
        idx_start = (t_win_start*num_train_timesteps - 1 ).long().to('cpu')
        alphas_cumprod_start = self.alphas_cumprod[idx_start].to(idx_start.device)
        idx_end = torch.clamp( (t_win_end*num_train_timesteps - 1 ).long(), min=0).to('cpu')
        alphas_cumprod_end = self.alphas_cumprod[idx_end].to(idx_end.device)
    ...
    def step(
        ...
        if self.config.prediction_type == "ddim_eps":
            ...
            lambda_t =  ( lambda_s.to(t_e.device) * (t_e - t_s) ) / ( lambda_s.to(t_e.device) *(t_c - t_s) + (t_e - t_c) )
            eta_t = ( eta_s.to(t_e.device) * (t_e - t_c) ) / ( lambda_s.to(t_e.device) *(t_c - t_s) + (t_e - t_c) )
    ...
    def inverse_step(
        ...
        if self.config.prediction_type == "ddim_eps":
            ...
            pred_velocity = ((1-lambda_s)*sample-eta_s*model_output)/(lambda_s*(t_s-t_e))
        ...
        if self.prev_v is None:
            ...
            pred_velocity = self.prev_v + (pred_velocity-self.prev_v)
            prev_sample = sample - dt * pred_velocity
        self.prev_v = pred_velocity

        if not return_dict:
            return (prev_sample,pred_velocity)
        return PeRFlowSchedulerOutput(prev_sample=prev_sample, pred_original_sample=None)
    


    ######################### The true ddcm function
    def ddcm_step(self,
        source_latents: torch.FloatTensor,
        latents: torch.FloatTensor,
        uncond_latent: torch.FloatTensor,
        source_noise_pred: torch.FloatTensor,
        noise_pred: torch.FloatTensor,
        null_noise_pred: torch.FloatTensor,
        timestep: int,
        )-> Union[PeRFlowSchedulerOutput, Tuple]:
        ...
        pred_win_end_tgt = lambda_t * latents + eta_t * noise_pred
        pred_velocity_tgt = (pred_win_end_tgt - latents) / (t_e - (t_s + c_to_s))

        pred_win_end_source = lambda_t * source_latents + eta_t * source_noise_pred
        pred_velocity_source = (pred_win_end_source - source_latents) / (t_e - (t_s + c_to_s))

        pred_win_end_null = lambda_t * latents + eta_t * null_noise_pred
        pred_velocity_null = (pred_win_end_null - latents) / (t_e - (t_s + c_to_s))
        ...
        prev_step = self.timesteps[idx+1] if (idx+1)<len(self.timesteps) else 0
        dt = (prev_step - timestep) / self.config.num_train_timesteps
        dt = dt.to(latents.device, latents.dtype)
        
        prev_sample = latents + dt * pred_velocity_tgt
        prev_sample_source = source_latents + dt * pred_velocity_source
        prev_sample_null = latents + dt * pred_velocity_null
  
        pred_x0 = uncond_latent + 1.0*(prev_sample_null-prev_sample_source) +1.0*(prev_sample-prev_sample_null)
